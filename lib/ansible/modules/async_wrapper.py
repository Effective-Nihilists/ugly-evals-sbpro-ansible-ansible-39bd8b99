...
def _run_module(wrapped_cmd, jid, job_path):
    ...
    try:
        ...
        if interpreter:
            # Fall back to the current Python interpreter if the shebang path does not exist
            if not os.path.exists(to_bytes(interpreter[0], errors='surrogate_or_strict')):
            ...
            if sub_pid:
                ...
                while os.waitpid(sub_pid, os.WNOHANG) == (0, 0):
                    ...
                    if remaining <= 0:
                        ...
                        time.sleep(1)
                        # Write structured timeout result to the job file
                        try:
                            tmp_job_path = job_path + ".tmp"
                            with open(tmp_job_path, "w") as jf:
                                jf.write(json.dumps({
                                    "failed": True,
                                    "msg": "timed out",
                                    "ansible_job_id": jid,
